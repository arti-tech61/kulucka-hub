    ...
    return (
        <form className="space-y-6" onSubmit={gonder}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div><label htmlFor={`${id}-firma`} className={etiket}>Firma Adı</label><input id={`${id}-firma`} className={alan} type="text" placeholder="Firmanız" value={g.firma} onChange={(e) => setG({ ...g, firma: e.target.value })} /></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div><label htmlFor={`${id}-email`} className={etiket}>E-posta Adresi</label><input id={`${id}-email`} className={alan} type="email" placeholder="ornek@firma.com" value={g.email} onChange={(e) => setG({ ...g, email: e.target.value })} /></div>
                <div><label htmlFor={`${id}-tel`} className={etiket}>Telefon Numarası</label><input id={`${id}-tel`} className={alan} required type="tel" placeholder="+90 555 000 0000" value={g.tel} onChange={(e) => setG({ ...g, tel: e.target.value })} /></div>
            </div>
            <div><label htmlFor={`${id}-hizmet`} className={etiket}>Hizmet</label>
                <select id={`${id}-hizmet`} className={`${alan} appearance-none`} value={g.hizmet} onChange={(e) => setG({ ...g, hizmet: e.target.value })}>
                    <option>Filo Kiralama</option><option>Acil Onarım</option><option>Bakım Sözleşmesi</option><option>Diğer</option>
                </select>
            </div>
            <div><label htmlFor={`${id}-mesaj`} className={etiket}>Mesaj</label><textarea id={`${id}-mesaj`} className={`${alan} resize-none`} rows={5} placeholder="Lojistik ihtiyacınızı açıklayın..." value={g.mesaj} onChange={(e) => setG({ ...g, mesaj: e.target.value })} /></div>
