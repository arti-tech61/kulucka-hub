export interface SiteIcerik {
    host: string;
    gaId: string;
    baslik: string;
    aciklama: string;
    h1: string;
    paragraflar: string[];
    hizmetler: string[];
    anaSite: { ad: string; url: string };
    bolge: string;
    bolumler?: { baslik: string; href: string }[];
}

export const siteler: SiteIcerik[] = [
    {
        host: "eklemliplatform.com.tr",
        gaId: "G-CFM36MYGHJ",
        baslik: "Eklemli Platform Kiralama | Zor Erişim Noktaları İçin Doğru Makine",
        aciklama: "12-43 metre eklemli (akrobat) platform kiralama. Dizel, elektrikli ve hibrit seçenekler; Türkiye geneli günlük, haftalık, aylık kiralama. Ücretsiz keşif.",
        h1: "Eklemli Platform Kiralama",
        paragraflar: [
            "Eklemli (akrobat) platformlar, düz teleskopik bomların ulaşamadığı noktalara \"kırılan kol\" yapısıyla erişir: çatı arkaları, cephe girintileri, ağaç ve tesisat üzerinden aşarak çalışma gerektiren her nokta.",
            "12 metreden 43 metreye uzanan eklemli platform filomuzla Türkiye genelinde günlük, haftalık ve aylık kiralama yapıyoruz. Dizel, elektrikli ve hibrit seçenekleri; iç mekân hassas zemin çalışmalarından şantiye koşullarına kadar her ihtiyaca uygun makineyi operatörlü veya operatörsüz teslim ediyoruz.",
            "Keşif ücretsizdir: çalışma yüksekliğinizi ve erişim engellerinizi paylaşın, size doğru bom tipini ve maliyet hesabını aynı gün iletelim.",
        ],
        hizmetler: ["12-16 m elektrikli eklemli platformlar", "18-26 m dizel eklemli platformlar", "28-43 m yüksek erişim eklemli bomlar", "Operatörlü kiralama ve nakliye", "Ücretsiz keşif ve bom tipi danışmanlığı"],
        anaSite: { ad: "Artı Platform", url: "https://www.artiplatform.com.tr" },
        bolge: "Türkiye geneli",
    },
    {
        host: "bandirmamanlift.net",
        gaId: "G-LDY8ZP140T",
        baslik: "Bandırma Manlift Kiralama | Balıkesir Kuzeyinde Hızlı Teslimat",
        aciklama: "Bandırma, Erdek, Gönen ve Biga hattında manlift kiralama. Liman, fabrika ve tersane işlerine uygun makas ve bomlu platformlar; İSG uyumlu, hızlı teslimat.",
        h1: "Bandırma Manlift Kiralama",
        paragraflar: [
            "Bandırma ve çevresinde (Erdek, Gönen, Biga hattı) manlift ihtiyacınız için bölgeye en yakın filodan hızlı teslimat sağlıyoruz.",
            "Liman lojistiği, fabrika bakımları, tersane işleri ve enerji tesisleri — Bandırma'nın sanayi dokusuna uygun makas ve bomlu platform seçeneklerini günlük veya proje bazlı kiralayabilirsiniz.",
            "Operatör belgelendirmesi, İSG uyumlu ekipman kontrolü ve nakliye organizasyonu bizden. Fiyat teklifi için makinenin çalışacağı yüksekliği ve zemini bildirmeniz yeterli.",
        ],
        hizmetler: ["Makaslı platform kiralama (8-22 m)", "Bomlu platform kiralama (16-43 m)", "Liman ve tersane çalışmalarına uygun ekipman", "Günlük ve proje bazlı kiralama", "Nakliye ve İSG evrak organizasyonu"],
        anaSite: { ad: "Artı Platform", url: "https://www.artiplatform.com.tr" },
        bolge: "Bandırma, Erdek, Gönen, Biga",
    },
    {
        host: "izmirforkliftkiralama.net",
        gaId: "G-E24F8SWEFE",
        baslik: "İzmir Forklift Kiralama | Günlük ve Uzun Dönem, Operatörlü Seçenek",
        aciklama: "İzmir genelinde 1,5-10 ton forklift kiralama. Dizel, LPG, elektrikli; depo içi triplex seçenekler. Kemalpaşa, Torbalı, Çiğli, Aliağa'ya aynı gün teslimat.",
        h1: "İzmir Forklift Kiralama",
        paragraflar: [
            "İzmir genelinde (Kemalpaşa, Torbalı, Çiğli, Aliağa sanayi bölgeleri dahil) 1,5 tondan 10 tona forklift kiralama. Dizel, LPG ve elektrikli modeller; depo içi dar alan çalışmaları için triplex asansörlü seçenekler.",
            "Saatlik yükleme boşaltma işlerinden aylık sözleşmeli tedarike kadar esnek kiralama modelleri sunuyoruz. Yük tablonuza ve zemin koşullarınıza göre doğru kapasiteyi öneriyor, makineyi bakımı yapılmış ve sertifikalı hâlde teslim ediyoruz.",
            "İzmir içi aynı gün teslimat çoğu modelde mümkündür.",
        ],
        hizmetler: ["1,5-3 ton elektrikli forkliftler (depo içi)", "3-5 ton dizel/LPG forkliftler", "7-10 ton ağır hizmet forkliftleri", "Operatörlü saatlik yükleme-boşaltma", "Aylık sözleşmeli uzun dönem kiralama"],
        anaSite: { ad: "Artı Platform", url: "https://www.artiplatform.com.tr" },
        bolge: "İzmir ve çevresi",
    },
    {
        host: "kutahyaplatform.com",
        gaId: "G-6C9FTW6S1G",
        baslik: "Kütahya Platform Kiralama | Seramik Sanayisinin Yükseklik Çözümü",
        aciklama: "Kütahya ve ilçelerinde 8-32 m makaslı ve bomlu platform kiralama. Seramik fabrikaları ve depolara uygun; Eskişehir filosundan hızlı sevkiyat.",
        h1: "Kütahya Platform Kiralama",
        paragraflar: [
            "Kütahya ve ilçelerinde makaslı ve bomlu platform kiralama. Seramik fabrikaları, depo raf sistemleri, cam ve çini atölyeleri ile kamu binalarının bakım işlerinde; 8-32 metre aralığında doğru platformu proje sürenize göre kiralıyoruz.",
            "Eskişehir filomuzdan hızlı sevkiyat sayesinde Kütahya'ya teslimat genellikle aynı gün planlanır.",
            "Elektrikli modellerle iç mekân, dizel modellerle açık saha çalışmalarına uygun çözümler; nakliye ve İSG evrakları organizasyona dahildir.",
        ],
        hizmetler: ["Makaslı platformlar (8-18 m, elektrikli/dizel)", "Bomlu platformlar (16-32 m)", "Fabrika içi hassas zemin çözümleri", "Proje süresine göre esnek kiralama", "Aynı gün sevkiyat planlaması"],
        anaSite: { ad: "Eskişehir Platform", url: "https://eskisehirplatform.com" },
        bolge: "Kütahya ve ilçeleri",
    },
    {
        host: "aydinplatform.net",
        gaId: "G-ZHS202Q2CZ",
        baslik: "Aydın Platform Kiralama | Efeler, Nazilli, Söke Hattında Hızlı Teslim",
        aciklama: "Aydın genelinde 8-32 m makaslı ve bomlu platform kiralama. Jeotermal tesisler, tarım sanayii ve inşaat işleri için; İzmir filosundan hızlı sevkiyat.",
        h1: "Aydın Platform Kiralama",
        paragraflar: [
            "Aydın merkez, Efeler, Nazilli, Söke ve Kuşadası hattında makaslı ve bomlu platform kiralama hizmeti veriyoruz. Jeotermal enerji tesisleri, zeytin ve incir işleme sanayii, oteller ve inşaat projeleri için 8-32 metre aralığında doğru makineyi seçiyoruz.",
            "İzmir filomuzdan Aydın'a sevkiyat çoğu makinede aynı gün planlanır; Kuşadası ve Didim'deki turizm tesislerinin sezon öncesi bakım işlerinde dönemsel kiralama modelleri sunuyoruz.",
            "Fiyat teklifi için çalışma yüksekliğinizi ve zemin koşulunuzu iletmeniz yeterli; keşif ücretsizdir.",
        ],
        hizmetler: ["Makaslı platformlar (8-18 m, elektrikli/dizel)", "Bomlu platformlar (16-32 m)", "Otel ve tesis bakımlarına dönemsel kiralama", "Tarım sanayii depo çalışmaları", "Aynı gün sevkiyat planlaması"],
        anaSite: { ad: "Artı Platform", url: "https://www.artiplatform.com.tr" },
        bolge: "Aydın, Efeler, Nazilli, Söke, Kuşadası",
    },
    {
        host: "bilecikplatform.com",
        gaId: "G-W503GFYREQ",
        baslik: "Bilecik Platform Kiralama | Bozüyük Sanayi Hattına Aynı Gün Teslim",
        aciklama: "Bilecik ve Bozüyük'te 8-32 m makaslı ve bomlu platform kiralama. Seramik ve metal sanayine uygun ekipman; Eskişehir filosundan hızlı sevkiyat.",
        h1: "Bilecik Platform Kiralama",
        paragraflar: [
            "Bilecik merkez, Bozüyük, Osmaneli ve Söğüt hattında makaslı ve bomlu platform kiralama. Seramik fabrikaları, metal işleme tesisleri ve depo projelerinde 8-32 metre aralığında çalışıyoruz.",
            "Eskişehir filomuza yakınlık sayesinde Bilecik'e teslimat genellikle aynı gün yapılır; kısa süreli bakım duruşları için günlük kiralama en çok tercih edilen modeldir.",
            "Elektrikli modeller iç mekân hassas zeminlerde, dizel modeller açık sahada kullanılır; nakliye ve İSG evrakları organizasyona dahildir.",
        ],
        hizmetler: ["Makaslı platformlar (8-18 m)", "Bomlu platformlar (16-32 m)", "Fabrika bakım duruşlarına günlük kiralama", "Depo ve raf sistemi kurulum çalışmaları", "Nakliye ve İSG evrak organizasyonu"],
        anaSite: { ad: "Eskişehir Platform", url: "https://eskisehirplatform.com" },
        bolge: "Bilecik, Bozüyük, Osmaneli, Söğüt",
    },
    {
        host: "bozuyukplatform.com.tr",
        gaId: "G-80XK5R3BN2",
        baslik: "Bozüyük Platform Kiralama | Sanayi Bölgesine En Yakın Filo",
        aciklama: "Bozüyük OSB ve çevresinde makaslı ve bomlu platform kiralama. Seramik, cam ve lojistik tesislerine hızlı teslimat; günlük ve proje bazlı kiralama.",
        h1: "Bozüyük Platform Kiralama",
        paragraflar: [
            "Bozüyük Organize Sanayi Bölgesi ve çevresindeki tesislere makaslı ve bomlu platform kiralama hizmeti veriyoruz. Seramik ve cam fabrikalarının bakım işleri, lojistik depoların raf ve aydınlatma çalışmaları ana uzmanlık alanımızdır.",
            "Eskişehir-Bozüyük hattındaki filomuz sayesinde teslimat çoğunlukla aynı gün gerçekleşir; acil bakım duruşlarında saatlik çözüm de sunuyoruz.",
            "8-32 metre aralığındaki makine parkımızdan zemin ve yükseklik koşulunuza uygun modeli ücretsiz keşifle belirliyoruz.",
        ],
        hizmetler: ["Makaslı platformlar (8-18 m)", "Bomlu platformlar (16-32 m)", "OSB içi acil bakım teslimatı", "Günlük ve proje bazlı kiralama", "Ücretsiz keşif"],
        anaSite: { ad: "Eskişehir Platform", url: "https://eskisehirplatform.com" },
        bolge: "Bozüyük OSB ve çevresi",
    },
    {
        host: "canakkaleplatform.com",
        gaId: "G-SKXSE7ZCN5",
        baslik: "Çanakkale Platform Kiralama | Biga Sanayi ve Merkeze Hızlı Teslim",
        aciklama: "Çanakkale, Biga ve Çan hattında 8-32 m makaslı ve bomlu platform kiralama. Enerji santralleri, seramik ve gıda tesislerine uygun ekipman.",
        h1: "Çanakkale Platform Kiralama",
        paragraflar: [
            "Çanakkale merkez, Biga, Çan ve Gelibolu hattında makaslı ve bomlu platform kiralama. Biga'daki enerji ve metal tesisleri, Çan seramik sanayii ve bölge genelindeki inşaat projeleri için 8-32 metre aralığında makine sağlıyoruz.",
            "Bandırma hattındaki filomuzdan Çanakkale'ye sevkiyat planlı günlerde ertesi gün teslim olarak organize edilir; proje bazlı uzun kiralamalarda makine sahada konumlanır.",
            "Rüzgâr santrali bakımları ve liman çalışmaları gibi özel işlerde İSG uyumlu ekipman ve belgeli operatör desteği veriyoruz.",
        ],
        hizmetler: ["Makaslı platformlar (8-18 m)", "Bomlu platformlar (16-32 m)", "Enerji santrali bakım çalışmaları", "Proje bazlı sahada konuşlu kiralama", "Belgeli operatör desteği"],
        anaSite: { ad: "Artı Platform", url: "https://www.artiplatform.com.tr" },
        bolge: "Çanakkale, Biga, Çan, Gelibolu",
    },
    {
        host: "platformmanisa.com",
        gaId: "G-BKS6Y4X2JD",
        baslik: "Manisa Platform Kiralama | OSB ve Turgutlu Hattına Aynı Gün Teslim",
        aciklama: "Manisa OSB, Turgutlu, Salihli ve Akhisar'da makaslı ve bomlu platform kiralama. Elektronik ve gıda sanayine uygun; İzmir filosundan hızlı sevkiyat.",
        h1: "Manisa Platform Kiralama",
        paragraflar: [
            "Manisa Organize Sanayi Bölgesi, Turgutlu, Salihli ve Akhisar hattında makaslı ve bomlu platform kiralama. Elektronik fabrikaları, gıda işleme tesisleri ve lojistik depoların yüksekte çalışma ihtiyaçlarında 8-32 metre aralığında hizmet veriyoruz.",
            "İzmir filomuzdan Manisa'ya teslimat çoğu makinede aynı gün planlanır; OSB içindeki tesislere vardiya saatlerine uyumlu gece teslimatı da organize edilir.",
            "Temiz üretim alanları için elektrikli, açık saha için dizel modeller; keşif ve kapasite danışmanlığı ücretsizdir.",
        ],
        hizmetler: ["Makaslı platformlar (8-18 m, elektrikli/dizel)", "Bomlu platformlar (16-32 m)", "OSB vardiyasına uyumlu teslimat", "Temiz alan (elektrikli) çözümleri", "Ücretsiz keşif ve kapasite danışmanlığı"],
        anaSite: { ad: "Artı Platform", url: "https://www.artiplatform.com.tr" },
        bolge: "Manisa OSB, Turgutlu, Salihli, Akhisar",
    },
    {
        host: "manisaforklift.com",
        gaId: "G-754SNQKTY7",
        baslik: "Manisa Forklift Kiralama | OSB İçi Saatlik ve Aylık Kiralama",
        aciklama: "Manisa genelinde 1,5-10 ton forklift kiralama. Dizel, LPG, elektrikli modeller; OSB tesislerine saatlik yükleme-boşaltma ve aylık sözleşmeli tedarik.",
        h1: "Manisa Forklift Kiralama",
        paragraflar: [
            "Manisa OSB, Turgutlu ve Akhisar bölgesinde 1,5 tondan 10 tona forklift kiralama. Elektronik ve gıda sanayinin yoğun sevkiyat trafiğine saatlik yükleme-boşaltma, depo operasyonlarına aylık sözleşmeli tedarik modeliyle yanıt veriyoruz.",
            "Kapalı üretim alanları için elektrikli ve triplex asansörlü modeller, açık saha ve rampa işleri için dizel/LPG seçenekleri mevcuttur.",
            "Makineler bakımı yapılmış ve sertifikalı teslim edilir; talep hâlinde belgeli operatör görevlendirilir.",
        ],
        hizmetler: ["1,5-3 ton elektrikli forkliftler (depo içi)", "3-5 ton dizel/LPG forkliftler", "7-10 ton ağır hizmet forkliftleri", "Saatlik yükleme-boşaltma operasyonu", "Aylık sözleşmeli tedarik"],
        anaSite: { ad: "Artı Platform", url: "https://www.artiplatform.com.tr" },
        bolge: "Manisa OSB, Turgutlu, Akhisar",
    },
    {
        host: "yuksektecalismarehberi.com",
        gaId: "G-QG5XZ2JHQQ",
        baslik: "Yüksekte Çalışma Rehberi | Platform Seçimi, İSG ve Mevzuat",
        aciklama: "Yüksekte güvenli çalışma rehberi: makaslı/bomlu platform ve telehandler seçimi, İSG mevzuatı, operatör belgeleri ve pratik kontrol listeleri.",
        h1: "Yüksekte Çalışma Rehberi",
        paragraflar: [
            "Bu rehber; şantiye şefleri, bakım mühendisleri ve İSG uzmanları için yüksekte çalışma ekipmanı seçimini sadeleştirir: hangi işte makaslı platform, hangi işte eklemli veya teleskopik bom, hangi işte telehandler doğru tercihtir — gerekçeleriyle anlatıyoruz.",
            "İçerik yol haritamız: platform tipi seçim rehberi, telehandler ile bomlu platform karşılaştırması, 6331 sayılı İş Sağlığı ve Güvenliği Kanunu kapsamında yüksekte çalışma yükümlülükleri, operatör belgelendirme süreçleri ve iş öncesi kontrol listeleri.",
            "Rehber içerikleri sahada makine kiralayan ve kullanan ekiplerin gerçek sorularından derlenmektedir; yeni bölümler yayınlandıkça bu sayfadan duyurulur.",
        ],
        hizmetler: ["Platform tipi seçim rehberi (makaslı / eklemli / teleskopik)", "Telehandler mi bomlu platform mu? Karşılaştırma", "Yüksekte çalışma İSG mevzuatı özetleri", "Operatör belgesi ve eğitim süreçleri", "İş öncesi kontrol listeleri (indirilebilir)"],
        anaSite: { ad: "Artı Platform", url: "https://www.artiplatform.com.tr" },
        bolge: "Türkiye geneli · bilgi kaynağı",
        bolumler: [
            { baslik: "Telehandler mi, bomlu platform mu? Makine seçim rehberi", href: "/rehber/telehandler-mi-bomlu-platform-mu" },
        ],
    },
    {
        host: "platformakademisi.com",
        gaId: "G-3GYGQ0FR9L",
        baslik: "Platform Akademisi | Operatör Eğitimi ve Yetkinlik Kaynağı",
        aciklama: "Yükseltici platform operatörlüğü eğitim kaynağı: IPAF ve MYK belgelendirme yolları, güvenli kullanım teknikleri, sınav hazırlık notları.",
        h1: "Platform Akademisi",
        paragraflar: [
            "Platform Akademisi, yükseltici platform (manlift) operatörlüğüne adım atanlar ve belgesini yenileyecekler için hazırlanan eğitim kaynağıdır. MYK Ulusal Yeterlilik ve IPAF belgelendirme yollarını, sınav içeriklerini ve başvuru adımlarını sade Türkçeyle anlatıyoruz.",
            "Güvenli kullanım teknikleri, günlük makine kontrolleri, devrilme ve sıkışma risklerine karşı saha kuralları gibi pratik konular; gerçek şantiye senaryoları üzerinden işlenir.",
            "Eğitim takvimi ve içerik duyuruları bu sayfadan yayınlanacaktır; kurumsal ekip eğitimleri için ana sitemizden bize ulaşabilirsiniz.",
        ],
        hizmetler: ["MYK operatör belgesi başvuru rehberi", "IPAF eğitimi nedir, nasıl alınır?", "Günlük makine kontrol listeleri", "Saha güvenlik kuralları ve senaryolar", "Kurumsal ekip eğitimi organizasyonu"],
        anaSite: { ad: "Artı Platform", url: "https://www.artiplatform.com.tr" },
        bolge: "Türkiye geneli · eğitim kaynağı",
    },
];

export function hostIcinSite(host: string): SiteIcerik | undefined {
    const temiz = host.toLowerCase().replace(/^www\./, "").split(":")[0];
    return siteler.find((s) => s.host === temiz);
}
