...
export const IZMIRMAKASLIPLATFORM_XYZ: Record<string, BespokeIcerik> = {
    ...
    "hizmet:makasli-platform-kiralama": {
        h1: "Makaslı Platform Kiralama: Sınıflar, Seçim Kriterleri ve Kemalpaşa OSB Depolarında Kullanım Kapsamı",
        giris:
            "Makaslı platform, adını dikey açılan makas mekanizmasından alan ve sepeti yalnızca düz bir çizgide, doğrudan yukarı-aşağı hareket ettiren bir yükseltici sınıfıdır. Bu, onu eklemli/teleskopik bom platformdan (sepeti yatayda da uzatabilen), sepetli örümcek platformdan (dar geçitlerde paletli hareket eden) ve dikey direk platformdan (tek kişilik, çok dar alan makinesi) ayıran temel özelliktir. Kemalpaşa OSB'deki lojistik depo müşterilerimizde bu sınıf işlerin büyük bölümünü karşılar çünkü depo içi hedeflerin neredeyse tamamı — raf sırası, tavan tesisatı, aydınlatma hattı — sepetin doğrudan altına yaklaşılabilen, engelsiz dikey noktalardır. Ancak \"makaslı platform kiralama\" tek bir ürün değil bir sınıf ailesidir: tahrik tipine göre akülü (kapalı alan, emisyonsuz) ve dizel/4x4 arazi tipi (açık saha, düzensiz zemin) olarak, şaseye göre standart ve dar geçit modeli olarak ayrılır. Bu sayfa bu sınıfın kapsamını, hangi işte doğru seçim olduğunu ve hangi işte olmadığını genel hatlarıyla tanımlar; belirli bir işe (depo içi, çelik konstrüksiyon, zor zemin) özgü ayrıntılar diğer hizmet sayfalarımızda derinleştirilir.",
        maddeler: [
            { baslik: "Dikey erişim ile yatay erişimi ayırmak", metin: "Makaslı platform seçiminin ilk testi şudur: hedefin altına, engelsiz biçimde doğrudan yaklaşılabiliyor mu? Yaklaşılabiliyorsa makaslı platform genellikle en verimli ve en ekonomik seçenektir çünkü mekanizması basit, kapasitesi yüksek ve kirası eklemli sınıfa göre daha düşüktür. Hedef bir boru hattı, sabit ekipman veya yapı çıkıntısı arkasındaysa ve sepetin yatayda uzanması gerekiyorsa makaslı platform yanlış araçtır; bu durumda eklemli/teleskopik bom platform değerlendirilir. Bu ayrım keşifte netleştirilmeden makine seçimi yapılmaz." },
            { baslik: "Tahrik tipini çalışma ortamına göre belirlemek", metin: "Kapalı depo veya bina içinde egzoz emisyonu kısıtı nedeniyle yalnızca akülü model kullanılır; dizel makine hiçbir gerekçeyle kapalı alana sokulmaz. Açık sahada zemin düz ve sağlamsa akülü model yine tercih edilebilir, ancak zemin hafriyat, kırmataş veya eğimliyse 4x4 dizel arazi tipi gerekir çünkü akülü modelin dar tekerleği ve düşük tırmanma kabiliyeti bu zeminde yetersiz kalır. Karar zeminin ve ortamın gerçek durumuna göre verilir, işin 'içeride mi dışarıda mı' olduğu tek başına yeterli bilgi değildir." },
            { baslik: "Şase genişliğini koridor ölçüsüyle eşleştirmek", metin: "Standart akülü makaslı platformun şase genişliği yaklaşık 1,15 metredir; bu ölçü çoğu geniş depo koridorunda ve açık sahada sorun yaratmaz. Dar raf arası, kapı genişliği kısıtlı bir bölüm veya asansör içinden geçiş gerekiyorsa 0,76-0,81 metre şaseli dar geçit modelleri gerekir. Dar modelin kapasitesi ve platform yüksekliği standart modele göre genellikle daha sınırlıdır; bu yüzden dar şase yalnızca geçiş zorunluluğu varsa tercih edilir, geniş alanda gereksiz yere kullanılmaz." },
            { baslik: "Kapasite ve çalışma yüksekliğini iş listesine göre hesaplamak", metin: "Doğru sınıf seçimi hedefin yüksekliğinden başlar ama orada bitmez: sepette kaç kişi çalışacak, hangi malzeme ve alet taşınacak da kapasiteyi belirler. İki kişi artı malzeme paleti taşınacaksa tek kişilik kapasiteye göre seçilmiş bir makine sahada yetersiz kalır. Çalışma yüksekliği (operatörün eriştiği en üst nokta) ile platform yüksekliği (sepet tabanı) arasındaki yaklaşık 1,8-2 metrelik fark da hesaba katılmazsa 'makine yetmiyor' şikâyeti sahada ortaya çıkar." },
            { baslik: "Genel amaçlı sınıfın sınırını bilmek", metin: "Makaslı platform, dar koridor içinde geniş bir sepet alanı ve yüksek kapasite sunduğu için işletmelerin çoğu iş tipinde ilk tercihidir, ama her işe uygun değildir. Merdiven boşluğu, asansör kuyusu gibi çok dar ve düzensiz geometrili alanlarda sepetli örümcek veya dikey direk platform daha uygun olabilir; geniş bir alanda tek bir noktadan çok yönlü erişim gerekiyorsa eklemli platform tercih edilir. Bu sınırları önceden bilmek, sahada yanlış makineyle vakit kaybetmeyi önler." },
        ],
        ekBolumler: [
            { baslik: "Makaslı platform alt sınıfları karşılaştırma tablosu", paragraflar: ["Aşağıdaki tablo, makaslı platform ailesindeki temel alt sınıfları ve tipik kullanım alanlarını gösterir. Kesin seçim sahanın zemin, ortam ve geçiş ölçüsüne göre netleşir."], tablo: { basliklar: ["Alt sınıf", "Tahrik", "Tipik ortam", "Şase/geçiş"], satirlar: [["Standart akülü makaslı", "Elektrik", "Kapalı depo, düz beton saha", "~1,15 m"], ["Dar geçit akülü makaslı", "Elektrik", "Dar koridor, asansör içi geçiş", "~0,76 – 0,81 m"], ["4x4 dizel arazi tipi makaslı", "Dizel", "Açık saha, hafriyat, eğimli zemin", "Geniş, yüksek tırmanma"], ["İtmeli (manuel) mini makaslı", "İtmeli", "Çok küçük iç mekan noktaları", "Dar, düşük kapasiteli"]] } },
            { baslik: "Makaslı platform hangi işte tercih edilmez", paragraflar: ["Sepetin hedefin altına doğrudan yaklaşamadığı, engel arkasındaki noktalarda makaslı platform yanlış sonuç verir; operatörün korkuluğa çıkması veya sepet dışına uzanması güvenlik ihlalidir ve bu durumda eklemli/teleskopik bom platform ya da farklı bir yöntem değerlendirilmelidir. Aynı şekilde çok dar, düzensiz geometrili veya tek kişilik erişim gereken noktalarda sepetli örümcek ya da dikey direk platform daha uygun sonuç verir.", "Bu sınırların iş talebinde erken belirtilmesi, sahaya yanlış makine sınıfının gelmesini ve ikinci bir sevkiyat gerekmesini önler. Hedefin fotoğrafı ve yaklaşım açısı paylaşıldığında doğru alt sınıf teklif aşamasında netleşir."] },
            { baslik: "Kemalpaşa OSB depolarında makaslı platformun ağırlığı", paragraflar: ["Kemalpaşa OSB'deki lojistik depo müşteri profilimizde iş taleplerinin büyük çoğunluğu makaslı platform sınıfıyla karşılanır; bunun nedeni depo içi hedeflerin genellikle raf sırası, tavan tesisatı ve aydınlatma hattı gibi doğrudan dikey erişilebilir noktalar olmasıdır. Bu profil şirketin kuruluş odağını da belirlemiştir ve diğer hizmet sayfalarımızdaki depo içi, çelik konstrüksiyon ve zor zemin senaryoları hep bu temel sınıfın farklı ortamlardaki uygulamalarıdır.", "Buna rağmen her talep otomatik olarak makaslı platforma yönlendirilmez; hedefin gerçek konumu ve zemin/ortam koşulu her seferinde ayrı değerlendirilir. Yanlış sınıfın verimsiz biçimde kullanılması hem güvenlik hem maliyet açısından doğru sonuç vermez."] },
        ],
        sss: [
            { soru: "Makaslı platform ile eklemli (bom) platform arasındaki fark nedir?", cevap: "Makaslı platform sepeti yalnızca dikey doğrultuda, düz bir çizgide yukarı-aşağı hareket ettirir; hedefin doğrudan altına engelsiz yaklaşılabildiğinde en verimli ve genellikle en ekonomik seçimdir. Eklemli platform ise kollarını hem yukarı hem yatayda katlayıp uzatabilir; hedef bir engelin arkasındaysa veya sepetin yandan yaklaşması gerekiyorsa bu sınıf gerekir. İki sınıf da 'platform kiralama' başlığı altında anılsa da mekanizmaları ve doğru kullanım alanları farklıdır." },
            { soru: "Kapalı depoda dizel makaslı platform kullanılabilir mi?", cevap: "Hayır, kapalı alanda egzoz emisyonu kısıtı nedeniyle dizel makine hiçbir gerekçeyle kullanılmaz; yalnızca akülü model devreye girer. Açık sahada ise zemin düz ve sağlamsa akülü model yine tercih edilebilir, ancak hafriyat, kırmataş veya eğimli zeminde 4x4 dizel arazi tipi gerekir çünkü akülü modelin tekerleği ve tırmanma kabiliyeti bu zeminde yetersiz kalır. Karar ortamın gerçek durumuna göre verilir." },
            { soru: "Dar koridorda hangi makaslı platform modeli gerekir?", cevap: "Standart akülü makaslı platformun şase genişliği yaklaşık 1,15 metredir; koridor bundan darsa 0,76-0,81 metre şaseli dar geçit modelleri seçilir. Bu ölçü, raf ayağından raf ayağına en dar noktadan alınmalıdır çünkü palet taşmaları etkin genişliği düşürebilir. Dar modelin kapasitesi standart modele göre genellikle daha sınırlıdır, bu yüzden yalnızca geçiş zorunluluğu varsa tercih edilir." },
            { soru: "Platform yüksekliği ile çalışma yüksekliği neden farklı ifade ediliyor?", cevap: "Platform yüksekliği sepetin tabanının yerden yüksekliğini, çalışma yüksekliği ise operatörün sepetten uzanarak eriştiği en üst noktayı ifade eder; aradaki fark yaklaşık 1,8-2 metredir. Hedefin gerçek yüksekliği ölçülüp bu farkın üzerine eklenmezse seçilen makine sahada yetersiz kalabilir. Bu yüzden metraj bilgisi talep aşamasında hedefin doğrudan ölçümüyle netleştirilir, kabaca tahminle sınıf belirlenmez." },
            { soru: "Makaslı platform hangi durumda yanlış tercih olur?", cevap: "Hedefin altına doğrudan, engelsiz biçimde yaklaşılamıyorsa (boru hattı, sabit ekipman veya yapı çıkıntısı hedefin altını kapatıyorsa) makaslı platform yanlış sonuç verir; operatörün korkuluğa çıkması veya sepet dışına uzanması kabul edilmez. Böyle durumlarda eklemli/teleskopik bom platform ya da hedefin konumuna göre farklı bir sınıf değerlendirilir. Bu ayrım keşifte netleştirilir, sahada davranışla telafi edilmez." },
            { soru: "Sepette iki kişi ve malzeme paleti taşınacaksa kapasite nasıl belirlenir?", cevap: "Kapasite yalnızca hedefin yüksekliğine göre değil, sepette çalışacak kişi sayısı ve taşınacak malzeme/alet ağırlığına göre de belirlenir. İki kişi ve bir malzeme paleti taşınacaksa tek kişilik kapasiteye göre seçilmiş bir makine sahada yetersiz kalır; bu bilgi talep aşamasında paylaşılırsa doğru kapasiteli model ilk seferde teslim edilir ve sahada ikinci makine ihtiyacı doğmaz." },
        ],
        kaynak: "Elle yazıldı — 2026-08-14. Makaslı platform sınıflandırması (tahrik tipi, şase ölçüleri, platform/çalışma yüksekliği farkı) sektör standardı genel bilgidir (EN 280 sınıf tanımları). Kemalpaşa OSB müşteri profili ve depo içi kullanım ağırlığı önceki bölge sayfamızdaki saha bilgisiyle tutarlıdır; tesis adı veya işletmeye özel rakam uydurulmamıştır.",
    },

    "hizmet:manlift-kiralama": {
        h1: "Manlift Kiralama: Personel Yükseltici Sınıfları ve Doğru Makinenin Seçimi",
        giris:
            "\"Manlift\" günlük dilde çoğu zaman tek bir makineyi değil, insanı yükseğe çıkaran tüm ekipman ailesini tanımlamak için kullanılır: makaslı platform, eklemli/teleskopik bom platform, sepetli örümcek platform, dikey direk platform ve kısmen sepetli forklift ataşmanları bu şemsiye terimin altına girer. Bu geniş kullanım pratik bir sorun yaratır — bir müşteri \"manlift istiyoruz\" dediğinde, işin gerçek doğası (dikey mi yatay erişim mi, açık mı kapalı mı, dar mı geniş alan mı) netleşmeden hangi sınıfın kastedildiği belli olmaz. Bu sayfa, şirketimizin sunduğu manlift ailesinin tamamını tanımlar ve hangi işte hangi sınıfın doğru seçim olduğunu gösteren genel bir karar çerçevesi sunar. Kemalpaşa OSB'deki lojistik depo ve sanayi müşterilerimizde talebin büyük kısmı makaslı platform sınıfıyla karşılanır çünkü depo içi hedefler genellikle doğrudan dikey erişilebilir noktalardır; ancak hedefin engel arkasında olduğu, çok dar geçit gerektiren veya tekerlekli aracın giremediği noktalarda diğer manlift sınıfları devreye girer. Bu sayfa makaslı platforma özgü ayrıntılara değil, tüm ailenin kapsamına ve aralarındaki seçim mantığına odaklanır; belirli bir sınıfın derinlemesine anlatımı için ilgili hizmet sayfalarımız kullanılır." ,
        maddeler: [
            { baslik: "Manlift talebini önce erişim yönüne göre ayırmak", metin: "Bir manlift talebi geldiğinde ilk soru hedefin nasıl bir erişim gerektirdiğidir: doğrudan dikey mi, yoksa bir engelin arkasına yatay uzanma mı gerekiyor? Dikey erişim makaslı platformun alanıdır. Yatay uzanma, boru hattı veya sabit ekipman arkasına ulaşma gerekiyorsa eklemli/teleskopik bom platform gündeme gelir. Bu ayrım netleşmeden \"manlift\" kelimesiyle gelen bir talebe doğru makine önerilemez; keşifte veya en azından fotoğraf paylaşımıyla bu netleştirilir." },
            { baslik: "Tekerlekli aracın giremediği noktalar için alt sınıfları bilmek", metin: "Bazı noktalara standart tekerlekli bir makine (makaslı veya bom) hiç giremez: çok dar bir kapı, merdiven boşluğu, hassas zeminli bir alan. Bu durumlarda paletli sepetli örümcek platform (dar geçitlerden geçip geniş alanlara ulaşabilen) veya tek kişilik dikey direk platform (çok kompakt, hafif) değerlendirilir. Bu iki sınıf standart makaslı/bom ailesinden farklı bir çözüm kategorisidir ve yalnızca geçiş veya zemin kısıtı gerçekten varsa tercih edilir." },
            { baslik: "Açık saha ile kapalı alanı tahrik seçiminde ayırmak", metin: "Hangi manlift sınıfı seçilirse seçilsin, tahrik tipi ortama göre belirlenir: kapalı alanda emisyon kısıtı nedeniyle yalnızca akülü/elektrikli model kullanılır, açık ve düzensiz zeminde ise dizel veya 4x4 arazi tipi model gerekebilir. Bu kural makaslı, eklemli ve sepetli örümcek sınıflarının hepsi için geçerlidir; sınıf seçimi ile tahrik seçimi birbirinden bağımsız iki karardır ve ikisi de sahanın gerçek koşuluna göre verilir." },
            { baslik: "Operatörlü mü kendin kullan mı sorusunu erken sormak", metin: "Manlift talebinde sınıf kadar önemli bir diğer soru operatör ihtiyacıdır. İşletmenin kendi personeli yeterlilik belgesine sahipse makine operatörsüz teslim edilebilir; personel yoksa veya iş karmaşıksa (çok noktalı rota, özel erişim koşulları) operatörlü kiralama tercih edilir. Bu tercih makine sınıfından bağımsızdır ama teklif ve program üzerinde doğrudan etkilidir; bu yüzden talep aşamasında netleştirilir." },
            { baslik: "Yanlış sınıf talebini düzeltmenin maliyetini bilmek", metin: "\"Manlift\" genel adıyla gelen bir talepte, sahaya varan makinenin gerçek ihtiyaca uymaması hem zaman hem nakliye maliyeti demektir; ikinci bir sevkiyat gerekir ve iş programı gecikir. Bunu önlemenin yolu, talep aşamasında hedefin fotoğrafı, yaklaşım yönü, geçiş ölçüsü ve zemin bilgisinin paylaşılmasıdır. Bu bilgiler netleştiğinde doğru manlift sınıfı ilk seferde sahaya gelir." },
        ],
        ekBolumler: [
            { baslik: "Manlift ailesi sınıf karşılaştırma tablosu", paragraflar: ["Aşağıdaki tablo, \"manlift\" şemsiye terimi altındaki temel sınıfları ve hangi erişim ihtiyacına karşılık geldiğini gösterir."], tablo: { basliklar: ["Sınıf", "Erişim yönü", "Tipik kullanım", "Kısıt"], satirlar: [["Makaslı platform", "Yalnız dikey", "Depo içi, raf, tavan tesisatı", "Hedef altı boş olmalı"], ["Eklemli/teleskopik bom", "Dikey + yatay", "Engel arkası, çok yönlü erişim", "Daha geniş çalışma alanı ister"], ["Sepetli örümcek (paletli)", "Dikey + kısmi yatay", "Dar geçit, hassas zemin", "Kapasite genelde daha düşük"], ["Dikey direk platform", "Yalnız dikey, kompakt", "Çok dar, tek kişilik nokta", "Düşük yükseklik/kapasite"], ["4x4 dizel arazi tipi (makaslı/bom)", "Zemine göre değişir", "Açık saha, hafriyat, eğim", "Kapalı alanda kullanılmaz"]] } },
            { baslik: "\"Manlift\" kelimesiyle gelen talebi netleştirme adımları", paragraflar: ["Sahadan \"manlift lazım\" bilgisiyle gelen bir talepte önce hedefin erişim yönü (dikey mi, engel arkası mı), sonra ortam (kapalı mı açık mı, zemin ne durumda), sonra geçiş ölçüsü (kapı, koridor genişliği) ve son olarak operatör ihtiyacı sırasıyla sorulur. Bu dört bilgi netleşmeden bir sınıf önerilmez; aksi hâlde sahaya yanlış makine gönderilme riski yükselir.", "Bu netleştirme süreci genellikle telefon görüşmesi ve birkaç fotoğrafla birkaç dakika içinde tamamlanır; karmaşık veya birden fazla farklı erişim tipi içeren işlerde kısa bir saha keşfi önerilir. Netleşen bilgiyle teklif tek seferde doğru sınıf ve tahrik tipiyle hazırlanır."] },
            { baslik: "Kemalpaşa OSB'de manlift talebinin dağılımı", paragraflar: ["Kemalpaşa OSB'deki müşteri talepleri arasında manlift adıyla gelen işlerin büyük çoğunluğu, netleştirme sonrası makaslı platform sınıfına karşılık gelir; bunun nedeni bölgedeki depo ve hafif sanayi tesislerinin çoğunda hedefin doğrudan dikey erişilebilir olmasıdır. Kalan kısım ise engel arkası erişim gerektiren çelik konstrüksiyon veya tesisat işlerinde eklemli platforma, çok dar geçitli özel noktalarda sepetli örümcek veya dikey direk platforma yönlenir.", "Bu dağılım şirketin hizmet yelpazesini de şekillendirmiştir: makaslı platform ana envanterde en geniş yer tutarken, diğer manlift sınıfları ihtiyaç bazlı tedarik edilir. Talebin hangi sınıfa karşılık geldiği her zaman sahanın gerçek koşuluna göre, önceden varsayılmadan belirlenir."] },
        ],
        sss: [
            { soru: "\"Manlift\" ile \"makaslı platform\" aynı şey mi?", cevap: "Hayır, manlift günlük dilde insanı yükseğe çıkaran tüm ekipman ailesini kapsayan şemsiye bir terimdir; makaslı platform bu ailenin yalnızca dikey erişim sağlayan bir alt sınıfıdır. Eklemli/teleskopik bom, sepetli örümcek ve dikey direk platform da manlift ailesine girer ama farklı erişim yönlerine ve farklı ortamlara hizmet eder. \"Manlift istiyoruz\" talebi geldiğinde önce hangi alt sınıfın gerektiği netleştirilir." },
            { soru: "İşimiz için hangi manlift sınıfı gerektiğini nasıl anlarız?", cevap: "Karar dört bilgiye dayanır: hedefin erişim yönü (doğrudan dikey mi, engel arkası mı), ortam (kapalı mı açık mı ve zemin durumu), geçiş ölçüsü (kapı/koridor genişliği) ve operatör ihtiyacı. Bu dört bilgi netleşmeden bir sınıf önerilmez. Hedefin fotoğrafı ve kısa bir açıklama talep aşamasında paylaşılırsa doğru sınıf genellikle birkaç dakika içinde belirlenir." },
            { soru: "Hangi durumlarda makaslı platform yerine eklemli (bom) manlift gerekir?", cevap: "Hedef bir boru hattı, sabit ekipman veya yapı çıkıntısı gibi bir engelin arkasındaysa ve sepetin yatayda uzanması gerekiyorsa eklemli/teleskopik bom platform gerekir. Makaslı platform yalnız dikey hareket ettiği için bu tip erişimlerde operatörün korkuluğa çıkması veya sepet dışına uzanması gibi güvensiz çözümlere başvurmadan iş yapılamaz. Bu ayrım keşifte veya fotoğraf üzerinden netleştirilir." },
            { soru: "Manlift dar bir kapıdan veya koridordan geçemiyorsa alternatif nedir?", cevap: "Standart tekerlekli makaslı veya bom platform giremeyecek kadar dar bir geçiş varsa paletli sepetli örümcek platform veya çok kompakt dikey direk platform değerlendirilir. Sepetli örümcek dar geçitlerden geçip daha geniş bir alanda çalışma yüksekliğine ulaşabilir; dikey direk platform ise tek kişilik, düşük yükseklikli noktalarda tercih edilir. Hangisinin uygun olduğu geçiş ölçüsü ve hedef yüksekliğine göre belirlenir." },
            { soru: "Açık sahada manlift kiralarken tahrik tipi neden önemli?", cevap: "Açık sahada zemin düz ve sağlamsa akülü/elektrikli model yine kullanılabilir, ama hafriyat, kırmataş veya eğimli zeminde dizel ya da 4x4 arazi tipi model gerekir çünkü akülü modelin tekerleği ve tırmanma kabiliyeti bu zeminde yetersiz kalır. Bu kural sınıftan (makaslı, bom, sepetli örümcek) bağımsız olarak geçerlidir; sınıf seçimi ile tahrik seçimi ayrı ayrı, sahanın gerçek koşuluna göre verilir." },
            { soru: "Manlift operatörsüz mü yoksa operatörlü mü kiralanmalı?", cevap: "İşletmenin kendi personeli geçerli yeterlilik belgesine sahipse makine operatörsüz teslim edilebilir; personel yoksa, iş karmaşıksa veya çok noktalı bir rota planlanıyorsa operatörlü kiralama tercih edilir. Bu tercih makine sınıfından bağımsız bir karardır ve teklif hazırlığı ile program üzerinde doğrudan etkisi olduğu için talep aşamasında netleştirilmesi önerilir." },
        ],
        kaynak: "Elle yazıldı — 2026-08-14. \"Manlift\" teriminin günlük dilde şemsiye kullanım biçimi ve alt sınıfların (makaslı, eklemli, sepetli örümcek, dikey direk) genel tanımları sektör standardı bilgidir. Kemalpaşa OSB talep dağılımına ilişkin genel gözlem önceki bölge sayfamızdaki saha bilgisiyle tutarlıdır; kesin oran, tesis adı veya işletmeye özel rakam uydurulmamıştır.",
    },
};
